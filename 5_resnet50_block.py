...
Includes: Transformer Encoder Policy + L2 Baseline for comparison
"""
...
BATCH_SIZE = 128  # Smaller batch for ResNet-50 (more memory)
NUM_WORKERS = 4
IMG_SIZE = 224
TOKEN_DIM = 128
SUM_DIM = 64
...
def main():
    ...
    print("Methods: Transformer Encoder + L2 Baseline")
    print("="*60)
    ...
    print("="*60)
    print(f"{'Ratio':<8} {'L2 Acc':<12} {'Encoder Acc':<12} {'Δ (Enc-L2)':<12}")
    print("-"*44)
    for i, r in enumerate(EVAL_RATIOS):
        l2_acc = l2_results[i]["accuracy_after_ft"] * 100
        enc_acc = encoder_results[i]["accuracy_after_ft"] * 100
        delta = enc_acc - l2_acc
        print(f"{r:<8.1f} {l2_acc:<12.2f} {enc_acc:<12.2f} {delta:+.2f}")
