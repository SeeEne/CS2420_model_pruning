...
# =========================
OUT_DIR = "checkpoints"
RESULTS_FILE = "aznas_encoder_results.json"
CHECKPOINT_FILE = "aznas_encoder_checkpoint.json"
...
# Evaluation ratios
TINYIMAGENET_RATIOS = [0.1, 0.3, 0.5, 0.7, 0.9]
CIFAR100_RATIO = 0.5
...
def compute_aznas_scores_gated_blocks(model, x, gates):
    """
    Improved AZ-NAS Score Computation:
    1. Differentiable (No .detach(), no .item())
    2. Numerically Stable (+epsilon identity)
    3. Memory Efficient (Compute score per layer immediately)
    """
    # 必须开启梯度追踪，因为 x 后面会经过 gated blocks
    # 如果 x 不需要梯度（只训练 gates），这里 x 不需要 requires_grad，
    # 但中间的 feature map 'h' 会自动带有 grad_fn (因为 gates 有 grad)
    ...
    h = model.bn1(h)
    h = model.relu(h)
    # Stem 通常不计算分或者作为 Layer 0，这里简单起见只算 Block 的输出
    h = model.maxpool(h)
    ...
    for layer in [model.layer1, model.layer2, model.layer3, model.layer4]:
        for block in layer:
            ...
            # --- [改进点] 即时计算分数，不要存 huge features list ---
            # 此时 h 带有梯度信息 (connected to gates)
            
            b, c, fh, fw = h.shape
            # Reshape: [B, C, H, W] -> [B*H*W, C]
            # 这种重排会让 PyTorch 保持梯度链
            
            # [优化] 如果特征图太大，先降采样，节省显存
            if fh > 14 or fw > 14:
            # 使用自适应池化将空间尺寸压缩到 14x14 或更小
            # 这保留了特征的统计分布，但减少了 75%~90% 的像素点数量
                h_sub = F.adaptive_avg_pool2d(h, (14, 14))
                X = h_sub.permute(0, 2, 3, 1).reshape(-1, c)
            else:
                X = h.permute(0, 2, 3, 1).reshape(-1, c)
            
            # 为了防止显存爆炸，如果像素太多，可以随机采样一部分像素
            # 这里先保留全量，但需注意显存
            ...
            # 协方差 (Covariance)
            # 注意：分母用 Xc.shape[0] - 1 (unbiased) 或 Xc.shape[0] 均可
            n = Xc.shape[0]
            sigma = (Xc.T @ Xc) / max(1, n)
            
            # --- [关键改进] 数值稳定性 (Numerical Stability) ---
            # 特征值分解对重合特征值极其敏感，导致梯度爆炸 (NaN)
            # 加一个微小的单位矩阵扰动 (Jitter)
            jitter = 1e-5 * torch.eye(c, device=x.device)
            sigma = sigma + jitter
            
            # 特征值分解 (Symetric Eigendecomposition) -> 可微
            try:
                # eigh 比 eig 更快且数值更稳定（针对对称矩阵）
                s = torch.linalg.eigvalsh(sigma)
            except RuntimeError:
                # 极少数情况可能失败，作为兜底
            ...
            # 归一化特征值 -> 概率分布
            s = torch.relu(s) + 1e-12 # 再次防止 log(0)
            p = s / s.sum()
    ...
    if len(scores) > 0:
        ...
        # Progressivity: min(Layer_i+1 - Layer_i)
        # 鼓励每一层都增加信息量
        if len(scores) >= 2:
            diffs = scores_stack[1:] - scores_stack[:-1]
            # 使用软最小值 (SoftMin) 或直接 Min 均可，这里直接用 Min
    ...
    return {
        "expressivity": expressivity,   # Tensor with grad
        "progressivity": progressivity, # Tensor with grad
    }
...
def train_aznas_encoder(teacher, train_loader, val_loader, num_classes, save_path):
    ...
    for epoch in range(total_epochs):
        ...
        run_expr = run_prog = run_aznas = run_ratio = run_l1 = 0.0
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            # AZ-NAS loss instead of KD loss (block-wise gated)
            aznas_info = compute_aznas_scores_gated_blocks(student, x, gates)

            # Transform to positive bounded loss (targets 0 as expressivity increases)
            # Typical expressivity values are 2-6, progressivity can be negative or positive
            expr = aznas_info['expressivity']
            prog = aznas_info['progressivity']

            # exp(-x/scale): high expressivity -> low loss (approaching 0)
            loss_expr = torch.exp(-expr / 5.0)  # expr=5 -> 0.37, expr=10 -> 0.14
            # Penalize negative progressivity, reward positive
            loss_prog = torch.exp(-prog / 1.0)  # prog=1 -> 0.37, prog=-1 -> 2.72

            loss_aznas = loss_expr + 0.1 * loss_prog  # Positive, bounded
            ...
            run_expr += expr
            run_prog += prog
            run_aznas += float(loss_aznas)
            run_ratio += float(loss_ratio)
            run_l1 += float(loss_l1)

            if (i + 1) % 50 == 0:
                print(f"[Ep {epoch + 1}/{total_epochs}][{i + 1}] "
                      f"expr={run_expr / 50:.2f} prog={run_prog / 50:.3f} "
                      f"loss_az={run_aznas / 50:.3f} ratio²={run_ratio / 50:.5f} temp={gate_temp:.2f}")
                run_expr = run_prog = run_aznas = run_ratio = run_l1 = 0.0
